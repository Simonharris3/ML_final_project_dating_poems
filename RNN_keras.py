...
def main():
    ...
    model.add(layers.SimpleRNN(512, dropout=0.5, recurrent_dropout=0.5,return_sequences=True)) #output_dim is the number of neurons in the recurrent layer #256 neurons
    model.add(layers.SimpleRNN(512, dropout=0.5, recurrent_dropout=0.5,return_sequences=True))
    model.add(layers.SimpleRNN(512, dropout=0.5, recurrent_dropout=0.5))
    model.add(layers.Dense(num_label, activation='softmax'))
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    
    callbacks = [EarlyStopping(monitor='val_loss', patience=50),
             ModelCheckpoint(filepath='best_RNN_keras.h5', monitor='val_loss', save_best_only=True)]
